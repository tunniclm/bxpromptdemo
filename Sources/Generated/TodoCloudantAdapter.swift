...
public class TodoCloudantAdapter: TodoAdapter {
    let client: CouchDBClient
    var database: Database {
        // TODO: Properly ensure database exists
        client.createDB("todo") { _, _ in }
    ...
    public func findAll(onCompletion: @escaping ([Todo], Swift.Error?) -> Void) {
        database.retrieveAll(includeDocuments: true) { maybeDocuments, error in
            ...
            do {
                let models = try documents.map { _, document -> Todo in
                    ...
                    let model = try Todo(json: modelJSON)
    ...
    public func create(_ model: Todo, onCompletion: @escaping (Todo?, Swift.Error?) -> Void) {
    ...
    public func findOne(_ maybeID: String?, onCompletion: @escaping (Todo?, Swift.Error?) -> Void) {
    ...
    public func update(_ maybeID: String?, with model: Todo, onCompletion: @escaping (Todo?, Swift.Error?) -> Void) {
        guard let id = maybeID else {
            return onCompletion(nil, AdapterError.invalidId(maybeID))
        }
        database.retrieve(id) { maybeDocument, error in
    ...
    public func delete(_ maybeID: String?, onCompletion: @escaping (Todo?, Swift.Error?) -> Void) {
        guard let id = maybeID else {
            return onCompletion(nil, AdapterError.invalidId(maybeID))
        }
        database.retrieve(id) { maybeDocument, error in
